...
import { deriveStrikeStatus, expiryOf } from '@/lib/strikes/status';
    ...
    <div>
      ...
      <div className="profile-grid">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-lg)' }}>
           ...
           {/* Strike History */}
           {(() => {
             const strikes = [...person.strikes].sort((a, b) => new Date(b.issued_at).getTime() - new Date(a.issued_at).getTime());
             const now = new Date();
             const status = deriveStrikeStatus(strikes.map(s => ({ issuedAt: s.issued_at, leadershipApproved: s.leadership_approved })), now);
             ...
             return (
               <div className="card">
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--space-md)', marginBottom: 'var(--space-lg)', flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-md)' }}>
