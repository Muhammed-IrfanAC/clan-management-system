...
### Environment
`.env.local` (dev/prod) and `.env.testing` hold: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `JWT_SECRET`, `COC_API_TOKEN`, optional `COC_API_PROXY_URL` (recommended — the CoC API requires IP allowlisting, so a fixed-IP proxy is used), optional `CRON_SECRET` (bearer token for `/api/cron/sync`), and optional `DISCORD_WEBHOOK_URL` (global fallback for Discord notifications).
